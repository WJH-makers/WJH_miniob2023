...
public:
  ...
  const std::vector<int>           &value_amount() const { return value_amount_; }

private:
  Table                           *table_ = nullptr;
  const std::vector<const Value *> values_;  // TODO when to delete ?
  const std::vector<int>           value_amount_;
};
